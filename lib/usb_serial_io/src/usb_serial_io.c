 ...
 */
void usbio_init(const uint8_t n) {
    buf_size = n;
 ...
 */
int usbio_print(const uint32_t ms_to_wait, const char* str) {
 ...
 */
int usbio_print_multi(const uint32_t ms_to_wait, const uint8_t num, const char* str, ...) {
    if (num == 0) return 0;

    int first_neg = 0;
    bool has_neg = false;

    int current_max = usbio_print(ms_to_wait, str);
    ...
    for (uint8_t i = 1; i < num; i++) {
        ...
        int res = usbio_print(ms_to_wait, next_str);
 ...
 * 
 * @param ms_to_wait 最大的阻塞等待时间（毫秒）。
 * @return usbio_rx_data_t 包含读取到的字节数和数据指针。如果不为 0，数据指针指向通过 malloc() 分配的内存，调用者处理完毕后【必须】使用 free((void*)data) 手动释放内存避免内存泄漏。
 */
usbio_rx_data_t usbio_read(const uint32_t ms_to_wait) {
    uint8_t *data = (uint8_t *)malloc(buf_size);
    if (data == NULL) {
        ESP_LOGE(DEFAULT_TAG, "Failed to allocate RX buffer");
        usbio_rx_data_t ret = { -1, NULL };
        return ret;
    }
    ...
    if (len <= 0) {
        free(data);
        usbio_rx_data_t ret = { len, NULL };
        return ret;
    }

    data[len] = '\0';

    usbio_rx_data_t ret = { len, data };
    return ret;
}

/**
 * @brief 释放由 usbio_read 函数分配的 USB 接收数据内存。
 * 
 * @param rx_data 指向包含待释放内存指针的结构体的指针，释放后结构体数据会被清空。
 */
void usbio_rx_data_destruct(usbio_rx_data_t* rx_data) {
