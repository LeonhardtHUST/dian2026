# FAT 存储库说明 (Storage Library)

这是一个基于 ESP-IDF FAT 文件系统 (具有磨损均衡/Wear Levelling能力) 的轻量级文件系统操作封装库。它为设备提供了一个简单易用的持久化存储接口，支持常见的文件读写、追加、删除和信息查询等功能。
...
1. **分区表 (Partition Table)**:
   要使用此库，您的 ESP32 设备需要在自定义的分区表 (`partitions.csv`) 中包含一个名为 `storage` 的 `fat` 数据分区。例如：
   ```csv
...
| 函数名称                 | 功能描述                                   |
|--------------------------|------------------------------------------|
| `storage_init()`         | 初始化并挂载 FAT 文件系统，基路径为 `/fatfs`。 |
...
void app_main() {
    ...
    // 2. 写入文件
    const char *test_data = "Hello, ESP32 FAT Storage!";
    storage_write_file("test.txt", test_data);
...
- 所有传入的文件名不需要加前缀，直接传入文件名即可（例如：`"config.json"`，库内部会自动转为 `/fatfs/config.json`）。
- **内存泄漏警告**：`storage_read_file` 为了灵活性会在内部分配足够的内存来装载文件数据，请**务必在处理完数据后调用 `free()` 释放内存**。
- FAT 支持目录结构，如果需要在子目录操作，可确保格式化挂载配置正确时带上目录前缀。(默认为根目录)
